...
class SynthesizerTrnCopy15(nn.Module): # TODO: Comment out this class
  """
  Synthesizer copy that uses speaker embeddings from another model
  """
  ...
  def infer(self, x, x_lengths, g=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths)
  ...
  def voice_conversion(self, y, y_lengths, sid_src, g_tgt):
    # assert self.n_speakers > 0, "n_speakers have to be larger than 0."
    g_src = self.emb_g(sid_src).unsqueeze(-1)
    # g_tgt = self.emb_g(sid_tgt).unsqueeze(-1)
    z, m_q, logs_q, y_mask = self.enc_q(y, y_lengths, g=g_src)
